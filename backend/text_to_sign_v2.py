"""
Text-to-sign translation with real human sign language video retrieval.

Maps English words/phrases to ASL-style gloss labels and retrieves
corresponding sign language videos from the dataset. Returns a structured 
sign sequence with video URLs for the frontend.
"""
...
_video_metadata_cache = None


def _load_video_metadata() -> dict[str, Any]:
    """Load video metadata from the video management system."""
    ...
    metadata_file = os.path.join(project_dir, "static", "signs", "videos_metadata.json")
    ...
    _video_metadata_cache = {"videos": {}, "sources": []}
    return _video_metadata_cache
...
def _find_video_url(gloss: str, english: str = "") -> str | None:
    ...
    Checks both the video metadata system and the signs directory.
    ...
    Returns:
        URL path like "/static/signs/videos/hello.mp4" or "/static/signs/hello.png"
        or None if not found
    """
    # First, check the video metadata system for real videos
    metadata = _load_video_metadata()
    videos = metadata.get("videos", {})
    
    # Try to find by exact English word match first
    if english:
        english_clean = english.lower().strip()
        if english_clean in videos:
            filename = videos[english_clean].get('file')
            if filename:
                return f"/static/signs/videos/{filename}"
    
    # Try to find by gloss variations
    candidates = []
    if gloss:
        candidates.append(gloss.lower())
        candidates.append(gloss.lower().replace("_", "-"))
        candidates.append(gloss.lower().replace("-", ""))
    ...
    for candidate in candidates:
        if candidate in videos:
            filename = videos[candidate].get('file')
            if filename:
                return f"/static/signs/videos/{filename}"
    
    # Fallback: check for image files in signs directory
    ...
    if gloss:
        fallback_candidates.append(gloss.lower())
        fallback_candidates.append(gloss.lower().replace("_", "-"))
    
    if english:
        fallback_candidates.append(english.lower())
        fallback_candidates.append(english.lower().replace(" ", "-"))
        fallback_candidates.append(english.lower().replace(" ", "_"))
...
def process_text_to_sign(text: str) -> dict[str, Any]:
    """
    Main entry point for text-to-sign translation.
    
    Prefers notebook-backed implementation when available, falls back to
    gloss mapping with video retrieval.
    """
